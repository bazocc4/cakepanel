<?php
	...
	$detail_type = "text";
	if(strpos(strtolower($validation), 'is_email') !== FALSE)
	{
		$detail_type = 'email';
	}
	else if(strpos(strtolower($validation), 'is_numeric') !== FALSE)
	{
		$detail_type = 'number';
	}
	else if(strpos(strtolower($validation), 'is_url') !== FALSE)
	{
		$detail_type = 'url';
	}
...
<div class="control-group" <?php echo (empty($display)?'':'style="display:none"'); ?>>            
	...
	<div class="controls">
		<input <?php echo ($detail_type=='number'?'step="0.01"':''); ?> <?php echo (!empty($readonly)?'readonly="true"':''); ?> <?php echo $required; ?> class="input-xlarge <?php echo $shortkey; ?> <?php echo $classtitle; ?>" type="<?php echo $detail_type; ?>" placeholder="<?php echo $placeholder; ?>" value="<?php echo (isset($_POST['data'][$model][$counter]['value'])?$_POST['data'][$model][$counter]['value']:$value); ?>" name="data[<?php echo $model; ?>][<?php echo $counter; ?>][value]"/>
		<p class="help-block">
            <?php echo $p; ?>
        </p>
	</div>
