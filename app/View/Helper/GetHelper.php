	...
	function outputConverter($inputType , $value , $myImageTypeList = NULL)
	{
		...
		switch ($inputType) 
		{
			...
			case 'checkbox':
				...
				break;
			case 'image':
				$result = '<div class="thumbs">'.(empty($popup)?$this->Html->link($this->Html->image('upload/thumb/'.$value.'.'.$myImageTypeList[$value]),'/img/upload/'.$value.'.'.$myImageTypeList[$value],array("escape"=>false)):$this->Html->image('upload/thumb/'.$value.'.'.$myImageTypeList[$value])).'</div>';
				break;
			case 'radio':
			case 'dropdown':
